      ...
      <main className="mx-auto max-w-[1500px] px-4 sm:px-6 py-8 sm:py-12 md:px-12">
        ...
        <div className="mt-16 sm:mt-24 space-y-0 mb-24 sm:mb-32 border-t border-ink/10">
          {articles.map((article, i) => (
            <div
              key={i}
              className="flex flex-col md:flex-row items-start md:items-center justify-between py-8 sm:py-12 border-b border-ink/10 px-4 sm:px-6 -mx-4 sm:-mx-6 rounded-2xl"
              aria-label={article.title}
            >
              <div className="flex-1">
                <div className="text-[10px] uppercase tracking-[0.35em] text-violet-deep mb-3 sm:mb-4">
                  {article.category}
                </div>
                <h2 className="font-display text-2xl sm:text-3xl md:text-4xl text-ink max-w-3xl leading-snug text-balance">
                  {article.title}
                </h2>
              </div>
